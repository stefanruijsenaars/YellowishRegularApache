...
function parse(item, idx, result, parsed = new Set([])) {
  ...
  [
       ${item.value
         .map((x) => parse(result[x], idx, result, parsed))
         .join(",\n    ")}
  ]`;
  }
  if (item.type === "object") {
    ...
    return `OBJECT ${idx}:
  {
    ${item.value
      .map(
        (x) =>
          parse(result[x.key], x.key, result, parsed) +
          " -> " +
          parse(result[x.value], x.value, result, parsed),
      )
      .join(",\n    ")}
  }`;
  }
    ...
    if (lines[lines.length - 1] && lines[lines.length - 1].type === "input") {
      try {
        ...
        setLines([
          ...
          {
            type: "output",
            value: parse(response.data.result[0], 0, response.data.result),
          },
        ]);
      } catch (e) {
        setIsSubmitting(false);
        ...
        if (e.response && e.response.data && e.response.status === 500) {
          setLines([
            ...lines,
            { type: "output", value: "SERVER ERROR: " + e.response.data },
          ]);
        }
      }
    }
